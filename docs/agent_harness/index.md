...
The {class}`~open_afps.provers.agent.prover.AgentProver` (and, by extension, the
{doc}`NuminaProver <../provers/numina>`) supports three agent harnesses. All three
implement the {class}`~open_afps.provers.agent.harness.Harness` interface and are
registered in {data}`~open_afps.provers.agent.harness.HARNESSES`.
...
{class}`~open_afps.provers.agent.prover.AgentProverConfig`'s `harness` field (one of
