...
- {doc}`provers` — `open_atp.provers`: the prover registry ({class}`~open_atp.provers.PROVERS`, {func}`~open_atp.provers.get_prover`, {func}`~open_atp.provers.available_provers`), the `AutomatedProver` base, its `ProofResult` output, and the concrete candidate generators.
