...
- {doc}`datasets` — `open_atp.datasets`: download the public Lean benchmark datasets (PutnamBench, FATE) as task directories ({func}`~open_atp.datasets.download_dataset`).
