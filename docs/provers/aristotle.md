...
The {class}`~open_afps.provers.aristotle.AristotleProver` wraps Harmonic's hosted
...
the shared {class}`~open_afps.core.verifier.Verifier` does the final check in a local
...
{attr}`~open_afps.provers.aristotle.AristotleProverConfig.api_key_env` (default
