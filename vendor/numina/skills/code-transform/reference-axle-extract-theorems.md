...
```bash
uv run --no-project .claude/skills/cli/axle.py extract-theorems CONTENT --environment ENV [OPTIONS]
```
...
```bash
uv run --no-project .claude/skills/cli/axle.py extract-theorems mathlib_file.lean --environment lean-4.28.0
uv run --no-project .claude/skills/cli/axle.py extract-theorems "theorem foo : 1 + 1 = 2 := by norm_num" --environment lean-4.28.0
```
