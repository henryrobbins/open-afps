...
```bash
uv run --no-project .claude/skills/cli/axle.py repair-proofs CONTENT --environment ENV [OPTIONS]
```
...
```bash
uv run --no-project .claude/skills/cli/axle.py repair-proofs proof.lean --environment lean-4.28.0
uv run --no-project .claude/skills/cli/axle.py repair-proofs proof.lean --environment lean-4.28.0 --repairs apply_terminal_tactics --terminal-tactics "grind,omega,simp,rfl"
uv run --no-project .claude/skills/cli/axle.py repair-proofs proof.lean --environment lean-4.28.0 --names my_theorem
```

## Notes

- Run `uv run --no-project .claude/skills/cli/lean_check.py FILE` first to identify which theorems are broken before calling repair-proofs.
