...
```bash
uv run --no-project .claude/skills/cli/axle.py simplify-theorems CONTENT --environment ENV [OPTIONS]
```
...
```bash
uv run --no-project .claude/skills/cli/axle.py simplify-theorems proof.lean --environment lean-4.28.0
uv run --no-project .claude/skills/cli/axle.py simplify-theorems proof.lean --environment lean-4.28.0 --simplifications remove_unused_tactics,remove_unused_haves
```

## Notes

- Only run on proofs that already compile successfully (use `uv run --no-project .claude/skills/cli/lean_check.py FILE` first).
