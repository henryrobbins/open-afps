...
| Tool | Skill | Purpose |
|------|-------|---------|
| `uv run --no-project .claude/skills/cli/lean_check.py FILE` | [verification](../verification/SKILL.md) | Diagnose compilation errors and pinpoint failing lines |
| `uv run --no-project .claude/skills/cli/axle.py sorry2lemma` | [code-transform](../code-transform/SKILL.md) | Extract sorry into standalone lemma with `--reconstruct-callsite` |
...
Run `uv run --no-project .claude/skills/cli/lean_check.py FILE` again.
...
Run `uv run --no-project .claude/skills/cli/axle.py sorry2lemma` with:
...
Run `uv run --no-project .claude/skills/cli/lean_check.py FILE` one last time.
