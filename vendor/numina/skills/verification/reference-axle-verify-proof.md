...
```bash
uv run --no-project .claude/skills/cli/axle.py verify-proof FORMAL_STATEMENT CONTENT --environment ENV [OPTIONS]
```
...
```bash
uv run --no-project .claude/skills/cli/axle.py verify-proof "theorem foo : 1 + 1 = 2 := by sorry" proof.lean --environment lean-4.28.0
uv run --no-project .claude/skills/cli/axle.py verify-proof formal_stmt.lean candidate.lean --environment lean-4.28.0 --permitted-sorries helper_lemma
```
