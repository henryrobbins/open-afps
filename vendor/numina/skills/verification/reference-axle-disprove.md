...
```bash
uv run --no-project .claude/skills/cli/axle.py disprove CONTENT --environment ENV [OPTIONS]
```
...
```bash
uv run --no-project .claude/skills/cli/axle.py disprove conjecture.lean --environment lean-4.28.0
uv run --no-project .claude/skills/cli/axle.py disprove conjecture.lean --environment lean-4.28.0 --names my_conjecture --terminal-tactics "grind,omega,decide"
```
