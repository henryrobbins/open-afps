...
```bash
uv run --no-project .claude/skills/cli/hammer_premise.py GOAL [-n NUM_RESULTS]
```
...
```bash
uv run --no-project .claude/skills/cli/hammer_premise.py "⊢ ∀ n : ℕ, 0 ≤ n"
uv run --no-project .claude/skills/cli/hammer_premise.py "⊢ Real.exp x > 0" -n 20
```
