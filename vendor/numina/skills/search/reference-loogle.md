...
```bash
uv run --no-project .claude/skills/cli/loogle.py QUERY [-n NUM_RESULTS]
```
...
```bash
uv run --no-project .claude/skills/cli/loogle.py "Real.sin"
uv run --no-project .claude/skills/cli/loogle.py "_ * (_ ^ _)" -n 10
uv run --no-project .claude/skills/cli/loogle.py "|- _ < _ → tsum _ < tsum _"
```
