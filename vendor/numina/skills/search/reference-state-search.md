...
```bash
uv run --no-project .claude/skills/cli/state_search.py GOAL [-n NUM_RESULTS]
```
...
```bash
uv run --no-project .claude/skills/cli/state_search.py "⊢ n + 0 = n"
uv run --no-project .claude/skills/cli/state_search.py "⊢ Nat.Prime p → p ∣ a * b → p ∣ a ∨ p ∣ b" -n 10
```
