...
### Rules
1. **All code must compile**: `uv run --no-project .claude/skills/cli/lean_check.py <file>` returns no severity-1 errors
...
```
uv run --no-project .claude/skills/cli/leanexplore.py "natural language description of what you need"
uv run --no-project .claude/skills/cli/loogle.py  "?f (?x + ?y) = ..." type pattern
uv run --no-project .claude/skills/cli/discussion_partner.py <<'EOF'
...
1. **Verify compilation**
   ```bash
   uv run --no-project .claude/skills/cli/lean_check.py <tmp file>
   ```
...
3. **Verify original compiles**
   ```bash
   uv run --no-project .claude/skills/cli/lean_check.py <original file>
   ```
...
All tools are local CLI scripts under `.claude/skills/cli/`. Read `.claude/skills/SKILL.md` and each sub-skill's `SKILL.md` / `reference-<tool>.md` for full parameters.

### High-Frequency Tools (use liberally!)

- `uv run --no-project .claude/skills/cli/lean_check.py <file>`: Compile + diagnostics (goals, hypotheses, errors) — run after every code change
- `uv run --no-project .claude/skills/cli/leanexplore.py QUERY`: Semantic mathlib search (supports up to 5 parallel queries)
- `uv run --no-project .claude/skills/cli/discussion_partner.py`: Ask Gemini/GPT for proof strategy hints

### Search & Guidance Tools (USE OFTEN!)

- `uv run --no-project .claude/skills/cli/leanexplore.py QUERY`: PRIMARY - semantic search for mathlib lemmas
- `uv run --no-project .claude/skills/cli/loogle.py`: Type pattern matching
- `uv run --no-project .claude/skills/cli/leanfinder.py` / `leansearch.py`: Alternative semantic search
- `uv run --no-project .claude/skills/cli/state_search.py`: Search by goal/state shape
- `uv run --no-project .claude/skills/cli/hammer_premise.py`: Premise retrieval for automation
- `uv run --no-project .claude/skills/cli/discussion_partner.py`: Ask Gemini for proof strategy hints
...
- For diagnostics and goal state on a given line, run `uv run --no-project .claude/skills/cli/lean_check.py <file>` and read its output
