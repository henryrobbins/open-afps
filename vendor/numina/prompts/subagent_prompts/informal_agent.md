...
Use `uv run --no-project .claude/skills/cli/informal_prover.py` or `uv run --no-project .claude/skills/cli/discussion_partner.py` to get guidance.
...
## Current Goal State (if available)
[paste diagnostics from `uv run --no-project .claude/skills/cli/lean_check.py <file>`]
...
### Use `uv run --no-project .claude/skills/cli/informal_prover.py`
...
### Use `uv run --no-project .claude/skills/cli/discussion_partner.py`
- For brainstorming approaches
- When the proof strategy is unclear
- For discussing trade-offs between approaches
- Backends: `gemini` / `gpt`

See `.claude/skills/llm/SKILL.md` and the corresponding `reference-*.md` for full parameters.
...
| Use `uv run --no-project .claude/skills/cli/informal_prover.py` for proofs | Use wrong tool | Right tool gives better results |
| Use `uv run --no-project .claude/skills/cli/discussion_partner.py` for brainstorming | Skip exploration | Open discussion finds creative approaches |
